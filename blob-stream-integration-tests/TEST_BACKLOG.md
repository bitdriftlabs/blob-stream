...
| IT-013 | done | bootstrap_dynamic_membership_scale_out_rebalances | Validate production bootstrap discovers new consumers from `consumer_group_membership` and rebalances | Start member A with `ConsumerConfigFactory::build_iterator_from_proto_config`; start member B with same group; at least one revocation observed and both members make forward progress while consumed set matches produced set | `RUST_LOG=off cargo nextest run -p blob-stream-integration-tests --test end_to_end_test bootstrap_dynamic_membership_scale_out_rebalances --no-fail-fast` | `RUST_LOG=blob_stream=trace,bd=trace cargo nextest run -p blob-stream-integration-tests --test end_to_end_test bootstrap_dynamic_membership_scale_out_rebalances --no-fail-fast` (pass, 3.55s); `RUST_LOG=off cargo nextest run -p blob-stream-integration-tests --test end_to_end_test bootstrap_dynamic_membership_scale_out_rebalances --no-fail-fast` (pass, 3.75s / 3.41s) |
| IT-014 | done | bootstrap_dynamic_membership_scale_in_after_expiry | Validate membership-store-driven scale-in when a member stops heartbeating | Start members A+B via production bootstrap; stop B without graceful shutdown; after membership TTL expiry A regains partitions and drains remaining records with no loss | `RUST_LOG=off cargo nextest run -p blob-stream-integration-tests --test end_to_end_test bootstrap_dynamic_membership_scale_in_after_expiry --no-fail-fast` | `RUST_LOG=blob_stream=trace,bd=trace cargo nextest run -p blob-stream-integration-tests --test end_to_end_test bootstrap_dynamic_membership_scale_in_after_expiry --no-fail-fast` (pass, 4.85s); `RUST_LOG=off cargo nextest run -p blob-stream-integration-tests --test end_to_end_test bootstrap_dynamic_membership_scale_in_after_expiry --no-fail-fast` (pass, 4.87s) |
| IT-015 | done | bootstrap_rebalance_with_membership_and_lease_faults | Validate rebalance convergence when membership and lease operations intermittently fail under fault injection | Use in-memory transport + store fault controller to inject membership heartbeat failures and consumer lease heartbeat/assign failures; membership/rebalance converges and final consumed set equals produced set; event log shows fault application and eventual successful ownership transitions | `RUST_LOG=off cargo nextest run -p blob-stream-integration-tests --test fault_injection_test bootstrap_rebalance_with_membership_and_lease_faults --no-fail-fast` | `RUST_LOG=blob_stream=trace,bd=trace cargo nextest run -p blob-stream-integration-tests --test fault_injection_test bootstrap_rebalance_with_membership_and_lease_faults --no-fail-fast` (pass, 6.98s); `RUST_LOG=off cargo nextest run -p blob-stream-integration-tests --test fault_injection_test bootstrap_rebalance_with_membership_and_lease_faults --no-fail-fast` (pass, 7.29s) |
| IT-016 | done | prefetch_rebalance_delayed_metadata_no_loss | Validate iterator prefetch under delayed metadata and consumer-group rebalance | Configure small `consumer.read.prefetch_max_bytes`, inject delayed metadata visibility, scale from 1->2 consumers, observe revocation, and verify all produced IDs are consumed with no loss | `RUST_LOG=off cargo nextest run -p blob-stream-integration-tests --test end_to_end_test prefetch_rebalance_delayed_metadata_no_loss --no-fail-fast` | `RUST_LOG=blob_stream=trace,bd=trace cargo nextest run -p blob-stream-integration-tests --test end_to_end_test prefetch_rebalance_delayed_metadata_no_loss --no-fail-fast` (pass, 5.62s); `RUST_LOG=off cargo nextest run -p blob-stream-integration-tests --test end_to_end_test prefetch_rebalance_delayed_metadata_no_loss --no-fail-fast` (pass, 5.78s) |
