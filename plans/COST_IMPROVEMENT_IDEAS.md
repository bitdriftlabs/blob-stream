...
It returns segment metadata for every virtual partition in that topic/window. Every consumer
currently repeats the query then filters locally. Hashing by virtual partition would not collapse
queries because each partition owner would still scan the same topic/window.

Hash `(topic, window_start)` within the consumer's local AZ/cluster instead. It aligns one local
cache owner with one DynamoDB partition key and spreads a high-throughput topic across local cache
owners as windows advance. Each AZ/cluster has an independent metadata cache by design, avoiding
cross-AZ cache traffic.

### Cache-Through Scan Design

Run a read-broker ring local to each AZ/cluster and rendezvous-hash `(topic, window_start)` to an
owner within that ring. The owner maintains a `WindowSnapshot`:
...
The broker returns matching metadata and filters partition indexes before serialization. The
consumer retains all stateful correctness decisions: Fresh/Recovering/Fast modes, cursors and source
checkpoints, seek and assignment, metadata visibility delay and ordering barriers, and cursor
commits. The cache owner is a shared query service, not a consumer-group coordinator.

### Refresh And Late-Metadata Correctness

For active windows, refresh a complete snapshot at a cadence no slower than the existing publication
and visibility safety horizon. For historical recovery windows, load on demand and retain a bounded
TTL/LRU snapshot so concurrent recovery shares the scan. Eviction is safe because a later request
reloads authoritative DynamoDB state.

The critical rule is that the cache must not store only one caller's `sk >= min_snowflake` response.
Metadata can become visible after a later Snowflake ID, including a row with a lower Snowflake ID.
Current recovery and Fast logic deliberately rescans to avoid losing that row.
...
Consumers continue applying metadata publication timestamps and their own visibility delay. A cache
does not make an otherwise too-new segment deliverable.

### Expected Cost Effect

Let $C_{local}$ be active consumer processes in one AZ/cluster for one topic, $W$ the windows each
scans, and $R$ refreshes per hour. A simplified local direct model is:
...
The realized RRU saving depends on page sizes and refresh behavior. Broker-to-consumer metadata
responses become the new fanout cost, so filter indexes to requested partitions, cap response/page
sizes, and measure bytes. A very large active window can otherwise exchange DynamoDB cost for broker
CPU and network pressure.

This idea reduces DynamoDB metadata queries and RRUs. It does not reduce S3 GETs by itself; Idea 2
can be combined later but should remain independently measurable.
...
Start with cache-through scanning. A tailer is a separate project requiring event delivery, replay,
checkpointing, cache-owner handoff, gap detection, and reconciliation semantics.

### Measurement, Tests, And Rollout

Measure cache hit/miss, refresh/full-refresh, eviction, and singleflight waiters; broker-issued
DynamoDB queries/RRUs; response bytes/pages/latency; memory; direct-Dynamo fallback; snapshot age;
and refresh failures.

Use shadow mode to compare cache responses with a direct DynamoDB query before allowing cached
metadata to control delivery. Verify eligible segment identity, partition index, sequence range,
byte range, compression, and publication timestamp.
...
Any implemented change that alters object layout, broker routing, metadata availability, or read
behavior must update `docs/design.md` in the same change. `docs/design.md` is the current
architecture contract; this document is a decision and implementation-planning aid.
...
2. Run focused unit tests and deterministic integration tests. Do not use wall-clock sleeps as
   success conditions; use lifecycle gates, manual time, or observable causal boundaries.
...
5. Monitor requests, RRUs/WRUs, object/range bytes, latency, broker resources, errors, fallback, and
   cache routing locality. Treat any cross-AZ cache route as a configuration error.
6. Require delivery-equivalence checks in shadow rollout: same virtual partition, sequence range,
   source metadata identity, and decoded payload.
7. Keep rollback compatible with every previously written metadata row and object. Consumers must
   treat blob keys as opaque so shared and topic-scoped objects coexist.
8. Promote only when observed savings exceed added broker compute, network, and operational cost
   with adequate latency and reliability headroom.
