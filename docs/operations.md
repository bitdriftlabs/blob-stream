...
| `blob_stream_broker_shared_cross_topic_blobs` | `false` | None; reduces time-triggered S3 object count | When any local topic becomes time-due, packs all available buffered non-draining topic sections into bounded shared objects while retaining one topic-local metadata row per section. Disable to stop creating shared objects; existing shared-key metadata remains readable. |
...
| Broker write path | `blob_stream_broker_shared_cross_topic_blobs`, `blob_stream_broker_max_segment_bytes` | Live | A time-due local partition pulls every available buffered non-draining local topic section into bounded shared objects and applies the positive serialized-object cap to future flush plans. Independently byte-triggered and lease-drain work remain local. |
