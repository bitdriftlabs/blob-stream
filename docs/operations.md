...
| `blob_stream_broker_shared_cross_topic_blobs` | `false` | None; reduces time-triggered S3 object count | Packs selected time-due topic sections into one object while retaining one topic-local metadata row per section. Disable to stop creating shared objects; existing shared-key metadata remains readable. |
...
| Broker write path | `blob_stream_broker_shared_cross_topic_blobs`, `blob_stream_broker_max_segment_bytes` | Live | Enables shared objects only for time-due work and overrides the positive serialized-object cap for future flush plans. Byte-trigger and lease-drain objects remain partition-local. |
